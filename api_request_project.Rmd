...
+ LAG- allows us to select a value that lags behind current value
+ DENSERANK allows us to rank results based upon partition 
+ ROWS allows us to grab rows preceeding our current value

In my queries, LAG is used to create a growth column.  Quarterly data can suffer from seasonality, therefore, we can use LAG set with 4, to compare the YOY(year over year) quarterly growth.  For exmaple, in our data lag,4 returns Q1 2018 if our current value is Q1 2019.  We can then use a growth formula to measure growth as percentage change.  Next I use DENSERANK to rank the results of our LAG querie. The next function builds a ranking based on growth, and cna be modified to look at industry level versus company level quarterly performance rankings.  The last function builds a three year rolling average.  It isn't really all that functional, it's meant more to demonstrate how one would apply such a query
...
+ As the directions suggested attempting a windows function tracking cumsum values for customers, this query mimics that workflow by creating a moving average of the yearly revenue over a window including the 2 years prior.
  ...
  + The function then uses that table with ROWS to build the 3 year moving average.  It adds casewhen in order to ensure a NUll resultfor the years 2009 and 2010(less than 3 years)
